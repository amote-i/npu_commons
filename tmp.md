...
def scenarios():
    ...
    S.append(sc("api_grid35424", B=9, T=7872, H=32, variant="api"))            # fix check: dense 35424
    S.append(sc("api_varlen_32840", lens=[262080, 512, 128], H=8, variant="api"))  # fix check: varlen 32840
...
def child_run_trial(cfg, args):
    # keep CPU BLAS tame should anything ever fall back to host
    ...
    if cfg["variant"] in ("plain", "reorder") and grid_size > 32768:
        print(f"[child] NOTE: direct kernel launch with {grid_size} > 2^15 programs -- "
              f"this is the UNFIXED canary path and is EXPECTED to hit the aicore "
              f"timeout; the workaround lives in solve_tril_npu (run variant=api "
              f"scenarios to verify it).", flush=True)
    ...
    try:
        if cfg["variant"] == "api":
            # end-to-end public API (carries the grid-split workaround) -- used
            # to confirm the fix on shapes whose unsplit grid is over 2^15
            for _ in range(max(1, cfg["repeat"])):
                Ai = solve_tril_npu(A=A_in, cu_seqlens=cu_seqlens, output_dtype=out_dtype)
                torch.npu.synchronize()
        else:
...
def torch_reference_inverse(A, cu_seqlens, B, T, H):
    ...
    Computed via the Neumann series sum_k (-A)^k on the NPU: A is strictly
    lower per 64-chunk hence nilpotent (A^64 = 0), so the series is exact.
    torch.linalg.inv is NOT used -- on this stack it falls back to CPU where
    OpenBLAS crashes on many-core hosts for large batched inversions.
    """
    ...
    def inv_bht(x):  # x: [b, h, t, BT]
        bb, hh, tt, bt = x.shape
        pad = (bt - tt % bt) % bt
        blk = torch.nn.functional.pad(x, (0, 0, 0, pad)).reshape(bb, hh, -1, bt, bt).to(torch.float32)
        neg = -blk
        p = torch.eye(bt, device=blk.device, dtype=torch.float32).expand_as(blk).contiguous()
        acc = p.clone()
        for _ in range(bt - 1):
            p = torch.matmul(p, neg)
            acc = acc + p
            if p.abs().max().item() < 1e-8:
                break
        return acc.reshape(bb, hh, -1, bt)[:, :, :tt, :]

    if cu_seqlens is None:
        return inv_bht(A.permute(0, 2, 1, 3)).permute(0, 2, 1, 3)
    parts = []
    cl = cu_seqlens.tolist()
    for s, e in zip(cl[:-1], cl[1:]):
        if e > s:
            parts.append(inv_bht(A[0, s:e].transpose(0, 1)[None])[0].transpose(0, 1))
        else:

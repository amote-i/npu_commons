...
# A single aicore launch deadlocks (task timeout, errType=0x4) once the total
# program count of the 2D triton grid crosses 2^15 on Ascend 910B -- reproduced
# in isolation with merge_16x16_to_64x64_inverse_kernel: 32544 blocks runs
# clean, 32832 hangs, both sides of the boundary verified against the device
# signature seen in production plogs. Launches are therefore split along one
# grid axis so each stays below this bound (kept under the verified-OK point).
_MAX_GRID_BLOCKS = 32000


def _split_grid(nt: int, bh: int, H: int, chunk_indices, is_varlen: bool):
    """Yield (nt_slice, bh_slice, chunk_indices_slice, batch_range) covering the
    [nt, bh] grid, keeping nt_slice * bh_slice <= _MAX_GRID_BLOCKS.

    varlen grids are sliced along the chunk axis (rows of chunk_indices, which
    the kernels index via program_id(0), so a sliced tensor needs no offset);
    dense grids along the folded batch*head axis (dim 0 of the tensors, sliced
    in whole batches). batch_range is (b0, b1) for a dense slice, else None.
    """
    if nt * bh <= _MAX_GRID_BLOCKS:
        yield nt, bh, chunk_indices, None
        return
    if is_varlen:
        step = max(1, _MAX_GRID_BLOCKS // bh)
        for s in range(0, nt, step):
            yield min(step, nt - s), bh, chunk_indices[s : s + step], None
    else:
        B = bh // H
        bstep = max(1, _MAX_GRID_BLOCKS // (nt * H))
        for b0 in range(0, B, bstep):
            b1 = min(b0 + bstep, B)
            yield nt, (b1 - b0) * H, None, (b0, b1)
...
def solve_tril_npu(
    ...
    for nt, bh, ci, brange in _split_grid(NT, B * H, H, chunk_indices, cu_seqlens is not None):
        A_s, Ad_s = (A, Ad) if brange is None else (A[brange[0] : brange[1]], Ad[brange[0] : brange[1]])
        solve_tril_16x16_kernel_paral_v3[nt, bh](
            A=A_s,
            Ad=Ad_s,
    ...
    for nt, bh, ci, brange in _split_grid(NT, B * H, H, chunk_indices, cu_seqlens is not None):
        A_s, Ad_s, Ai_s = (
            ...
            if brange is None
                ...
                A[brange[0] : brange[1]],
                Ad[brange[0] : brange[1]],
                Ai[brange[0] : brange[1]],
            )
        )
        merge_fn[nt, bh](
            A=A_s,
